...
| AI 大模型 | Ollama (llama3/qwen2) | 本地部署，RAG 智能问答 |
...
基于 RAG（Retrieval-Augmented Generation）的智能客服系统，支持商品咨询、订单查询等自然语言交互。

#### 技术架构
```
用户提问 → 语义解析 → 知识库检索 → LLM 生成 → 智能响应
                              ↑
                    Elasticsearch 向量库
                    + 商品/订单数据库
```
...
| 功能 | 说明 |
|------|------|
| 智能问答 | 支持自然语言查询商品信息、订单状态、优惠券使用规则 |
| 知识库检索 | 基于向量相似度匹配，快速定位相关知识文档 |
| 多轮对话 | 支持上下文记忆，连贯的对话体验 |
| 工具调用 | 自动调用订单查询、商品搜索等工具获取实时数据 |

#### 关键组件

- **MallAgent**：核心对话代理，协调对话流程
- **RagService**：检索增强生成服务，整合知识库与 LLM
- **ChatSession**：对话会话管理，维护上下文状态
- **KnowledgeBaseInitializer**：知识库初始化，加载商品/订单知识

#### 技术栈
- **LLM**：支持 Ollama 本地模型（如 llama3、qwen2）
- **向量检索**：Elasticsearch 向量相似度搜索
- **MCP**：Model Context Protocol，安全的工具调用
